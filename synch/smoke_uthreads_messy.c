...
typedef struct Handler_Package {
  struct Agent *agent; // contains condition variables to monitor
  enum Resource resource; // the resource
} handler_package_t;

// variables for smokers to wait on
uthread_cond_t tobacco_wakeup, match_wakeup, paper_wakeup; 
...
int paper_flag, match_flag, tobacco_flag; // TODO probably delete
...
void* resource_handler(package_t* pgk) {
  // pgk->resource is the resource to monitor
  ...
  // each waits on one resource

  // when the agent signals, two will wake up

  // each of these must lock

  // they then must update their associated flag (global)
  // and check the flags of the other two.                    
  //            -> does this violate the constraint that 
  //            -> "no other part of the system can know what resources are available"?
  //          Yes, it does.

  // if exactly one of the other flags are reset, wake the other thread by 
  // signaling the third resource condition variable

  // also, wake the appropriate smoker by signaling the condition variable they are waiting on
  // this probably needs to happen *after* the three resource handler threads have completed.

  // if both of the flags are reset, unset them all and continue
...
idea_feb26(void *a) {
  // modelled for smoker with matches
  ...
  while(1) {
    ...
    // uthread_cond_signal(match_reset); // monitored by successful thread before signalling smoke
  // instead of doing the above, just lock the mutex corresponding to match_reset, set it to 1, and unlock
  // then do the oposite (lock, set to 0, unlock) right after the below wait.

    uthread_cond_wait(agent->paper);
    uthread_mutex_lock(agent->mutex);
    if(/*some smoker has succeeded*/) {
      continue;
    }
    ...
    uthread_cond_wait(agent->tobacco);
    uthread_mutex_lock(agent->mutex);
    if(/* no smoker has succeeded*/) {
      // this smoker has now succeeded - record this somehow (set global int flag <success>?)
      ...
      /// since we were successful, that means that two signals have been given at this point: paper and tobacco.

      // Idea: add 3 more condition variables: t_reset, p_reset, m_reset for the threads to signal once they have reset.
      // issue with this: now have 2 more waits to deal with
    // alternate idea to above: use 3 global flags protected with mutexes and just busy wait until both are set.
    }
  }
}
